...
def parseArguments ():
    ...
    parser.add_argument ("--nb_ops", default=NB_OPS, type=int)
...
def exportResults (pids):
    ...
    print ("Energy host : ", sum (power_host), "J")
    print ("Energy procs : ", sum (power_procs), "J")
...
#    - pids: la liste de PIDS de l'application 'baseline'
#
def stopScaphandre (scaph, pids) :
    scaph.kill ()
    exportResults (pids)
...
def startBaselineLoad (functionName, nbCores, nbOps):
    processes = []
...
#    - nbOps: le nombre d'operation à effectuer par l'app baseline
#    - scenario: liste de dictionnaire décrivant un scénario (ex: [{'function_name' : 'sqrt', 'nb_cores' : 4, 'duration' : 10}])
#
def runExperiment (baselineFunction, nbCores, nbOps, scenario):
    processes = []
    scaph = startScaphandre ()
    time.sleep (1)

    (pids, processes) = startBaselineLoad (baselineFunction, nbCores, nbOps)
...
def main (arguments) :
    ...
    with open (arguments.scenario, "r") as f:
        ...
        runExperiment (arguments.func_name, arguments.nb_cores, arguments.nb_ops, scenario)
