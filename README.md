...
Réservation d'un serveur : `oarsub -r now -t inner=258151 -l walltime=1:00 -t deploy --project "lab-2024-imta-greenit"`
...
```
SERVER=$(oarstat -j JOBID -J | jq -r '.["JOBID"]["assigned_network_address"][0]')
kadeploy3 -m $SERVER ubuntu2204-nfs

ssh root@$SERVER apt update
ssh root@$SERVER apt install -y python3-pip cgroup-tools stress-ng
